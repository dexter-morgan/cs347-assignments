...
int next_process(struct processes *p, int np, int clock)
{
  ...
  for(j = 0; j < np; j++){
        ...
        if(p[j].arrival_time <= clock && p[j].burst_time < min_burst && p[j].finished!=1){
          next_pid = j;
...
void sjf(struct processes *p, int np)
{
  ...
  for(int i = 0; i < np; i++){
      ...
      int next_pid = next_process(p, np, clock);

      p[next_pid].response_time = clock - p[next_pid].arrival_time;
      clock += p[next_pid].burst_time;
